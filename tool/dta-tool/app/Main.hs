module Main where
    ...
    setConst :: [(Maybe (CDeclarator a), Maybe (CInitializer a), Maybe (CExpression a))] -> [(Maybe (CDeclarator a), Maybe (CInitializer a), Maybe (CExpression a))]
    setConst [(Just x, Just (CInitExpr (CConst (CStrConst _ ni3)) ni4), e)] =
    ...
    instrumentDecl d@(CDecl typeSpecifier declr node_info) = do 
        log $ isSource d
        let d' = CDecl typeSpecifier (setConst declr) node_info
        logPretty d'
        return d'
